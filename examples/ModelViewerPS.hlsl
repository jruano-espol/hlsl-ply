...
MRT main(VSOutput vsOutput)
{
	...
	float3 diffuseAlbedo = texDiffuse.Sample(defaultSampler, vsOutput.uv);
	float3 colorSum = 0;
	{
	...
	float3 normal;
	{
		normal = texNormal.Sample(defaultSampler, vsOutput.uv) * 2.0 - 1.0;
	...
	float specularMask = texSpecular.Sample(defaultSampler, vsOutput.uv).g;
